 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class completion_criteria_course_grade extends completion_criteria {

    /* @var int Criteria type constant [COMPLETION_CRITERIA_TYPE_GRADE] */
    public $criteriatype = COMPLETION_CRITERIA_TYPE_COURSE_GRADE;
    ...
    public static function fetch($params) {
        $params['criteriatype'] = COMPLETION_CRITERIA_TYPE_COURSE_GRADE;
    ...
    public function config_form_display(&$mform, $data = null) {
        $mform->addElement('checkbox', 'criteria_course_grade', get_string('enable'));
        $mform->addElement('text', 'criteria_course_grade_value', get_string('graderequired', 'completion'));
        $mform->disabledIf('criteria_course_grade_value', 'criteria_course_grade');
        $mform->setType('criteria_course_grade_value', PARAM_RAW); // Uses unformat_float.
        // Grades are stored in Moodle with 5 decimal points, make sure we do not accidentally round them
        // when setting the form value.
        $mform->setDefault('criteria_course_grade_value', format_float($data, 5));

        if ($this->id) {
            $mform->setDefault('criteria_course_grade', 1);
            $mform->setDefault('criteria_course_grade_value', format_float($this->gradepass, 5));
        }
    }
    ...
    public function update_config(&$data) {
        ...
        if (!empty($data->criteria_course_grade) && !empty($data->criteria_course_grade_passed)) {

            $this->course = $data->id;
            $formatedgrade = unformat_float($data->criteria_course_grade_value);

            // foreach ($data->criteria_course_grade_passed as $course) {
                
                $this->courseinstance = $data->criteria_course_grade_passed;
                $this->id = NULL;
    ...
    private function get_grade($completion) {
        $grade = grade_get_course_grade($completion->userid, $this->courseinstance);
        return $grade->grade;
    }
    ...
    WHERE
        cr.criteriatype = '.COMPLETION_CRITERIA_TYPE_COURSE_GRADE.'
